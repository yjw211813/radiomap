...
from compare.compare_utils import model_compare, model_ablation, model_ablation_nocars
...
def get_SAUNet_nosa_model(base_dir,load_epoch):
    ...
    SAUNet_save_dir = base_dir + r"/model_pth/SAUnet_nosa_nocars/"
...
def get_SAUNet_nofrac_model(base_dir,load_epoch):
    ...
    load_epoch = load_epoch
    SAUNet_save_dir = base_dir + r"/model_pth/SAUnet_nofrac_nocars/"
    SAUNet_checkpoint_path = os.path.join(SAUNet_save_dir, f"checkpoint_epoch_{load_epoch}.pth")
    SAUNet_checkpoint = torch.load(SAUNet_checkpoint_path, weights_only=True, map_location=device)
    print(f"加载历史数据load_epoch:{load_epoch}成功")
    model.load_state_dict(SAUNet_checkpoint['model_state_dict'])
...
def get_SAUNet_noMultiScale_model(base_dir,load_epoch):
    ...
    load_epoch = load_epoch
    SAUNet_save_dir = base_dir + r"/model_pth/SAUnetNoMultiScale_nocars/"
    SAUNet_checkpoint_path = os.path.join(SAUNet_save_dir, f"checkpoint_epoch_{load_epoch}.pth")
    SAUNet_checkpoint = torch.load(SAUNet_checkpoint_path, weights_only=True, map_location=device)
    print(f"加载历史数据load_epoch:{load_epoch}成功")
    model.load_state_dict(SAUNet_checkpoint['model_state_dict'])
...
def get_SAUNet_noStatistic_model(base_dir,load_epoch):
    ...
    load_epoch = load_epoch
    SAUNet_save_dir = base_dir + r"/model_pth/SAUnetNoSatistic_nocars/"
...
def get_SAUNet_model(base_dir,load_epoch):
    ...
    load_epoch = load_epoch
    SAUNet_save_dir = base_dir + r"/model_pth/SAUnet_nocars/"
...
if __name__ == "__main__":
    ...
    compare_dir = base_dir + r"/model_val_log/compare_ablation_nocars/"
    SAUNet_model = get_SAUNet_model(base_dir, 38)
    SAUNet_nosa = get_SAUNet_nosa_model(base_dir, 32)
    SAUNet_nofrac = get_SAUNet_nofrac_model(base_dir, 31)
    SAUNet_noMultiScale = get_SAUNet_noMultiScale_model(base_dir, 35)
    SAUNet_noStatistic = get_SAUNet_noStatistic_model(base_dir, 24)
    ...
    avg_metrics = model_ablation_nocars(models_dict, compare_dir, test_loader, device)