...
class fre_embedding(nn.Module):
    def __init__(self,out_dim,input_channels,length):
        ...
        self.down2 = Conv_DownSampling(C_out1)###  下采样 (100,C_out1,1,32)
        ...
        self.down3 = Conv_DownSampling(C_out1)  ###  下采样 (100,C_out1,1,16)
    ...
    def forward(self, x):
        ...
        out2 = self.conv_mixer2_2(self.conv_mixer2_1(self.down2(out1)))
...
###  time features : (100,2,1,64)
def fre_embedding_test():
    data = torch.randn(100,2,1,128)
    block = fre_embedding(out_dim = 20,input_channels = 2,length = 128)
    output_tensor = block(data)
    # 打印输出形状
    print(f"Input shape: {data.shape}")
    print(f"Output shape: {output_tensor.shape}")
...
##### wt2 features : (100,64,1,256)
class WT_embedding(nn.Module):
    def __init__(self,out_dim,input_channels,length):
        super(WT_embedding, self).__init__()
        ###  下采样 (100,C_out1,1,256)
        C_out1 = 128
        down_sample_times = 4
        self.Fra_conv = Fractal_inception(input_channel=input_channels, output_channel=C_out1)
        self.down1 = Conv_DownSampling(C_out1) ###  下采样 (100,C_out1,1,128)
        self.conv_mixer1_1 = conv_mixer_block(input_channel=C_out1, conv_mode='inception', kernel_size=[1, 3, 5, 7], dilated_num=1)
        self.conv_mixer1_2 = conv_mixer_block(input_channel=C_out1, conv_mode='inception', kernel_size=[1, 3, 5, 7], dilated_num=1)

        self.down2 = Conv_DownSampling(C_out1)###  下采样 (100,C_out1,1,64)
        self.conv_mixer2_1 = conv_mixer_block(input_channel=C_out1, conv_mode='inception', kernel_size=[1, 3, 5, 7], dilated_num=1)
        self.conv_mixer2_2 = conv_mixer_block(input_channel=C_out1, conv_mode='inception', kernel_size=[1, 3, 5, 7], dilated_num=1)

        self.down3 = Conv_DownSampling(C_out1)  ###  下采样 (100,C_out1,1,32)
        self.conv_mixer3_1 = conv_mixer_block(input_channel=C_out1, conv_mode='inception', kernel_size=[1, 3, 5, 7], dilated_num=1)
        self.conv_mixer3_2 = conv_mixer_block(input_channel=C_out1, conv_mode='inception', kernel_size=[1, 3, 5, 7], dilated_num=1)

        self.down4 = Conv_DownSampling(C_out1)  ###  下采样 (100,C_out1,1,16)
        self.conv_mixer4_1 = conv_mixer_block(input_channel=C_out1, conv_mode='inception', kernel_size=[1, 3, 5, 7],
                                              dilated_num=1)
        self.conv_mixer4_2 = conv_mixer_block(input_channel=C_out1, conv_mode='inception', kernel_size=[1, 3, 5, 7],
                                              dilated_num=1)
...
def WT_embedding_test():
    data = torch.randn(10,64,1,256)
    block = WT_embedding(out_dim = 20,input_channels = 64,length = 256)
    output_tensor = block(data)
    # 打印输出形状
    print(f"Input shape: {data.shape}")
    print(f"Output shape: {output_tensor.shape}")
...
class STFT_embedding(nn.Module):
    def __init__(self,out_dim,input_channels,length):
        ...
        self.down2 = Conv_DownSampling(C_out1)###  下采样 (100,C_out1,1,16)
        self.conv_mixer2_1 = conv_mixer_block(input_channel=C_out1, conv_mode='inception', kernel_size=[1, 3, 5, 7], dilated_num=1)
        self.conv_mixer2_2 = conv_mixer_block(input_channel=C_out1, conv_mode='inception', kernel_size=[1, 3, 5, 7], dilated_num=1)

        self.down3 = Conv_DownSampling(C_out1)  ###  下采样 (100,C_out1,1,8)
        self.conv_mixer3_1 = conv_mixer_block(input_channel=C_out1, conv_mode='inception', kernel_size=[1, 3, 5, 7], dilated_num=1)
        self.conv_mixer3_2 = conv_mixer_block(input_channel=C_out1, conv_mode='inception', kernel_size=[1, 3, 5, 7], dilated_num=1)
        vector_len = int(length/(2**down_sample_times))
        encoder_layer = nn.TransformerEncoderLayer(d_model = vector_len,
    ...
    def forward(self, x):
        ...
        out1 = self.conv_mixer1_2(self.conv_mixer1_1(self.Fra_conv(out1) )  )
        out2 = self.conv_mixer2_2(self.conv_mixer2_1(self.down2(out1)))
        out3 = self.conv_mixer3_2(self.conv_mixer3_1(self.down3(out2)))
        out4 = self.time_embed(out3.squeeze(2))
        out4_flat = out4.reshape(out4.size(0), -1)
...
def WT2_embedding_test():
    data = torch.randn(10,18,1,33)
    # 设置卷积核大小
    kernel_size = 2

    # 创建卷积层，设置合适的padding
    # conv = nn.Conv2d(9, 9, kernel_size=(1, kernel_size), stride=(1, 1),padding=(0, 0))
    embedding_layer = STFT_embedding(out_dim = 20,input_channels = 18,length = 33)
...
class CNN_embedding(nn.Module):
    def __init__(self,out_dim,input_channels,length):
        ...
        self.down2 = Conv_DownSampling(C_out2)#  下采样 (100,C_out2,1,64)
        self.Fra_conv3 = Fractal_inception(input_channel=C_out2, output_channel=C_out3)
        self.down3 = Conv_DownSampling(C_out3)#  下采样 (100,C_out3,1,32)
        self.Fra_conv4 = Fractal_inception(input_channel=C_out3, output_channel=C_out4)
        self.down4 = Conv_DownSampling(C_out4)  # 下采样 (100,C_out3,1,16)
    ...
    def forward(self, x):
        ...
        out3 = self.down3(self.Fra_conv3(out2))
...
def CNN_embedding_test():
    data = torch.randn(10,2,1,256)
    embedding_layer = CNN_embedding(out_dim = 20,input_channels = 2,length = 256)
    output_tensor = embedding_layer(data)
    # 打印输出形状
    print(f"Input shape: {data.shape}")
    print(f"Output shape: {output_tensor.shape}")
...
def CNN_mixers_embedding_test():
    data = torch.randn(10,2,1,256)
    embedding_layer = CNN_mixers_embedding(out_dim = 20,input_channels = 2,length = 256)
    output_tensor = embedding_layer(data)
    # 打印输出形状
    print(f"Input shape: {data.shape}")
    print(f"Output shape: {output_tensor.shape}")
...
def CNN_GRU_test():
    data = torch.randn(10, 2, 1, 256)
    embedding_layer = CNN_GRU_embedding(out_dim=20, input_channels=2, length=256)
    output_tensor = embedding_layer(data)
    # 打印输出形状
    print(f"Input shape: {data.shape}")
    print(f"Output shape: {output_tensor.shape}")
...
class wisig_classify_net(nn.Module):
    def __init__(self, emb_dim, shape_dict,class_num):
        super(wisig_classify_net, self).__init__()
        embedd_model_num = 7
        self.cat_dim = 1
...
def wave_classify_test():
    shape_dict = {
        'T_H': 24,  # Time Feature Height (对应 T_data 中的第一个维度)
        'T_W': 30,  # Time Feature Width (对应 T_data 中的第二个维度)
        'F_C': 2,  # Frequency Channels (对应 F_data 中的第一个维度)
        'F_Len': 128,  # Frequency Length (对应 F_data 中的第二个维度)
        'WT_C': 64,  # Wavelet Transform Channels (对应 WT_data 中的第一个维度)
        'WT_Len': 256,  # Wavelet Transform Length (对应 WT_data 中的第二个维度)
        'STFT_C': 18,  # STFT Channels (对应 STFT_data 中的第一个维度)
        'STFT_Len': 33,  # STFT Length (对应 STFT_data 中的第二个维度)
        'Ori_C': 2,  # Original Channels (对应 origin_data 中的第一个维度)
        'Ori_Len': 256  # Original Length (对应 origin_data 中的第二个维度)
    }
    # Data_S 模板
    ...
    my_net = wisig_classify_net(emb_dim = 64, shape_dict=shape_dict,class_num = 11)
