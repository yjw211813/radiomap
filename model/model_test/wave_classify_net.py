...
class fre_embedding(nn.Module):
    def __init__(self,out_dim,input_channels,length):
        ...
        self.down1 = Conv_DownSampling(C_out1) ###  下采样 (100,C_out1,1,32)
...
def fre_embedding_test():
    data = torch.randn(100,2,1,64)
    block = fre_embedding(out_dim = 20,input_channels = 2,length = 64)
    output_tensor = block(data)
    # 打印输出形状
    print(f"Input shape: {data.shape}")
    print(f"Output shape: {output_tensor.shape}")
...
def WT_embedding_test():
    data = torch.randn(10,32,1,256)
    block = WT_embedding(out_dim = 20,input_channels = 32,length = 256)
    output_tensor = block(data)
    # 打印输出形状
    print(f"Input shape: {data.shape}")
    print(f"Output shape: {output_tensor.shape}")
...
def WT2_embedding_test():
    data = torch.randn(10,9,1,33)
    # 设置卷积核大小
    kernel_size = 2

    # 创建卷积层，设置合适的padding
    # conv = nn.Conv2d(9, 9, kernel_size=(1, kernel_size), stride=(1, 1),padding=(0, 0))
    embedding_layer = STFT_embedding(out_dim = 20,input_channels = 9,length = 33)
...
class CNN_embedding(nn.Module):
    def __init__(self,out_dim,input_channels,length):
        ...
        self.down2 = Conv_DownSampling(C_out2)#  下采样 (100,C_out2,1,32)
        self.Fra_conv3 = Fractal_inception(input_channel=C_out2, output_channel=C_out3)
        self.down3 = Conv_DownSampling(C_out3)#  下采样 (100,C_out3,1,16)
        self.Fra_conv4 = Fractal_inception(input_channel=C_out3, output_channel=C_out4)
        self.down4 = Conv_DownSampling(C_out4)  # 下采样 (100,C_out3,1,8)
...
def CNN_embedding_test():
    data = torch.randn(10,2,1,128)
    embedding_layer = CNN_embedding(out_dim = 20,input_channels = 2,length = 128)
    output_tensor = embedding_layer(data)
    # 打印输出形状
    print(f"Input shape: {data.shape}")
    print(f"Output shape: {output_tensor.shape}")
...
def CNN_mixers_embedding_test():
    data = torch.randn(10,2,1,128)
    embedding_layer = CNN_mixers_embedding(out_dim = 20,input_channels = 2,length = 128)
    output_tensor = embedding_layer(data)
    # 打印输出形状
    print(f"Input shape: {data.shape}")
    print(f"Output shape: {output_tensor.shape}")
...
def CNN_GRU_test():
    data = torch.randn(10, 2, 1, 128)
    embedding_layer = CNN_GRU_embedding(out_dim=20, input_channels=2, length=128)
    output_tensor = embedding_layer(data)
    # 打印输出形状
    print(f"Input shape: {data.shape}")
    print(f"Output shape: {output_tensor.shape}")
...
class wave_classify_net(nn.Module):
    def __init__(self, emb_dim, shape_dict,class_num):
        super(wave_classify_net, self).__init__()
        embedd_model_num = 7
        self.cat_dim = 1
...
def wave_classify_test():
    ...
    my_net = wave_classify_net(emb_dim = 64, shape_dict=shape_dict,class_num = 11)
