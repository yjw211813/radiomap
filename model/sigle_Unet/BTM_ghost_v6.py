...
class BTM_Net_deeper(nn.Module):
    def __init__(self,input_shape, output_shape,C_list):
        super(BTM_Net_deeper, self).__init__()
        ...
        for i in range(len(C_list) - 1):
            ...
            layers.append(Res_Inception_ghost2D(C_in=C_list[i + 1], C_out=C_list[i + 1], kernel_sizes=[1, 3, 5, 7], dilated_num=1))
...
def BTM_Net_deeper_test():
    ...
    net = BTM_Net_deeper(input_shape,output_shape,C_list).to(device)
    output = net(condition_map)
    print(f"Output shape: {output.shape}")


class BTM_ghost_UNet_deeper(nn.Module):
    def __init__(self, input_shape, output_shape, C_down_list,attn_params):
        super(BTM_ghost_UNet_deeper, self).__init__()
        self.input_channel, self.input_H, self.input_W = input_shape
        ...
        for out_ch in C_down_list:
            self.encoder.append(nn.Sequential(
                ...
                Inception_ghost2D(C_in=out_ch, C_out=out_ch, kernel_sizes=kernel_sizes, dilated_num=1),
        ...
        for ch, factor, param in zip(attn_channels, attn_factors, attn_params):
            self.attentions.append(BTM_Net_deeper(
                input_shape,
...
def BTM_ghost_UNet_deeper_test():
    ...
    net = BTM_ghost_UNet_deeper(BTM_ghost_UNet_input_shape, BTM_ghost_UNet_output_shape,C_down_list,attn_params).to(device)
