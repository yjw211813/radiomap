...
class BTM_Net(nn.Module):
    def __init__(self,input_shape, output_shape,C_list):
        ...
        # 构建编码器
        layers = []
        layers.append(inception_ghost_sum(C_in=self.input_channel, C_out=C_list[0], kernel_list=[3], dilated_list=[1]))
        for i in range(len(C_list) - 1):
            layers.append(inception_ghost_sum(C_in=C_list[i], C_out=C_list[i + 1], kernel_list=[3],  dilated_list=[1]))
    ...
    def __init__(self, input_shape, output_shape, C_down_list,attn_params):
        ...
        for out_ch in C_down_list:
            self.encoder.append(nn.Sequential(
                inception_ghost_sum(C_in=in_ch, C_out=out_ch,  kernel_list=kernel_sizes, dilated_list=[1]),
                inception_ghost_sum(C_in=out_ch, C_out=out_ch, kernel_list=kernel_sizes, dilated_list=[1]),
                multiScaleConvDown(out_ch,kernel_sizes)
            ))
            in_ch = out_ch

        # 中心卷积层
        self.conv_center = fractal_conv(C_in=C_down_list[-1],C_out=C_down_list[-1],kernel_list = kernel_sizes,dilated_list = [1],inception_module = inception_sum)
        ...
        for i in range(len(C_down_list), 0, -1):
            ...
            self.decodes.append(
                nn.Sequential(
                    inception_ghost_sum(C_in=C_down_list[i] + C_down_list[i],C_out=C_down_list[i] + C_down_list[i], kernel_list=kernel_sizes, dilated_list=[1]),
                    inception_ghost_sum(C_in=C_down_list[i] + C_down_list[i], C_out=C_down_list[i],kernel_list=kernel_sizes, dilated_list=[1]),
