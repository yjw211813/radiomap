...
class SAUnet_old_nomultiscale(nn.Module):
    # 修改上卷积方法
    def __init__(self, input_shape, output_shape, C_down_list,attn_params):
        super(SAUnet_old_nomultiscale, self).__init__()
        self.input_channel, self.input_H, self.input_W = input_shape
...
def SAUnet_test():
    ...
    model = SAUnet_old_nomultiscale(BTM_ghost_UNet_input_shape, BTM_ghost_UNet_output_shape,C_down_list,attn_params).to(device)
    x = input_data
